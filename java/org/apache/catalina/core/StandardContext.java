...
public class StandardContext extends ContainerBase implements Context, NotificationEmitter {
    ...
    public boolean loadOnStartup(Container children[]) {
        ...
        for (int i = 0; i < children.length; i++) {
            ...
            //如果loadOnStartup小于0的化不加载，使用时再加载、初始化
            //如果loadOnStartup不小于0的化在应用启动时就加载并初始化这个servlet
            if (loadOnStartup < 0)
                continue;
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        try {
            ...
            if (ok) {
                ...
                /**
                 * HostConfig 执行部署逻辑的过程，其实就是创建了Context对象(具体为StandardContext)，并添加到Host中,
                 * 同时为context 添加监听器ContextConfig
                 * 具体代码
                 ...
                 等到执行StandardContext的startInternal()方法时触发监听器的执行
                 */
